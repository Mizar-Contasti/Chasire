...
```php
$session = false; || $session = updateSessionParameters($parameters);
...
triggerCall($session, $phoneNumber);
```
...
> - Retrieves the platform integration (web/phone-call).
